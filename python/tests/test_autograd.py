...
class TestAutograd(mlx_tests.MLXTestCase):
    ...
    def test_cummin_cummax_jvp(self):
        x = mx.array([2.0, 1.0, 0.5, 3.0])
        tan = mx.array([10.0, 20.0, 30.0, 40.0])

        _, jvps = mx.jvp(mx.cummin, [x], [tan])
        self.assertTrue(mx.allclose(jvps[0], mx.array([10.0, 20.0, 30.0, 30.0])))

        x = mx.array([2.0, 1.0, 3.0, 4.0])
        _, jvps = mx.jvp(mx.cummax, [x], [tan])
        self.assertTrue(mx.allclose(jvps[0], mx.array([10.0, 10.0, 30.0, 40.0])))
